...
function M.render(bufnr, range, translation, winid)
	...
	if opts.position == "below" then
		...
		vim.api.nvim_buf_clear_namespace(bufnr, NAMESPACE, range.end_row, range.end_row + 1)
	...
	elseif opts.position == "eol" then
		...
		vim.api.nvim_buf_clear_namespace(bufnr, NAMESPACE, range.start_row, range.start_row + 1)
	...
	elseif opts.position == "overlay" then
		vim.api.nvim_buf_clear_namespace(bufnr, NAMESPACE, range.start_row, range.end_row + 1)

		vim.api.nvim_buf_set_extmark(bufnr, NAMESPACE, range.start_row, range.start_col, {
