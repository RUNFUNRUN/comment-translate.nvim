...
function M.setup(opts)
	...
	local group = vim.api.nvim_create_augroup("CommentTranslate", { clear = true })
	...
	vim.api.nvim_create_autocmd({ "WinScrolled" }, {
		...
		callback = function()
			M.update_visible()
			last_comment_id = scanner.get_comment_id_at_cursor(vim.api.nvim_get_current_buf())
		end,
	})
	...
	vim.api.nvim_create_autocmd({ "CursorMoved" }, {
		group = group,
		callback = function()
			local current_id = scanner.get_comment_id_at_cursor(vim.api.nvim_get_current_buf())
...
function M.update_visible()
	...
	timer:start(
		...
		vim.schedule_wrap(function()
			...
			if vim.tbl_contains(config.options.exclude_filetypes, vim.bo[bufnr].filetype) then
				return
			end
