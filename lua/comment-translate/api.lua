...
function M.translate_deepl(text, callback)
	...
	local url = config.options.client.deepl.is_pro and "https://api.deepl.com/v2/translate"
	...
	curl.post(url, {
		...
		callback = vim.schedule_wrap(function(response)
			if response.status ~= 200 then
				callback(nil, "DeepL Error: " .. tostring(response.status) .. " " .. (response.body or ""))
				return
			end
